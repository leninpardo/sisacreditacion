...
class cambiar_clave extends Main{    
...
//        $xd=1+(int)$ct['cant'];
//               
        if($_GET["parametro"]=="alumno"){
            
            $sql="update alumno_cca set pass='".$_GET["pass"]."' where  alumno_cca.idalumno=".$_GET["id"]."";
            $stmt = $this->db->prepare($sql);
                    $stmt->execute();

        }
        
         if($_GET["parametro"]=="usuario"){
            
            $sql="update usuario_cca set pass='".$_GET["pass"]."' where  usuario_cca.idusuario=".$_GET["id"]."";
            $stmt = $this->db->prepare($sql);
                    $stmt->execute();

        }
        
         if($_GET["parametro"]=="docente"){
            
            $sql="update docente_cca set pass='".$_GET["pass"]."' where  docente_cca.iddocente=".$_GET["id"]."";
            $stmt = $this->db->prepare($sql);
                    $stmt->execute();
