...
class asignacionPS extends Main {
 ...
 Inner Join clasificacion_evento ON clasificacion_evento.id_clasificacion_evento = tipo_evento.id_clasificacion_evento
    where  evento.CodigoSemestre='" . $semestre_ultimo . "' and  (clasificacion_evento.id_clasificacion_evento=2  and  tipo_evento.idtipo_evento= 5) and evento.idevento_padre is null and " . $c . " like :query";
...
//        echo $sql;exit;
    ...
    function eliminar_docente($_c, $_ev) {
        $sql = "DELETE from detalle_asistencia_docente where detalle_asistencia_docente.CodigoProfesor='" . $_c . "' and detalle_asistencia_docente.idevento='" . $_ev . "'";
        $statement = $this->db->prepare($sql);
        $statement->execute();
    }
...
//                print_r($_P['apellido_E'][$keys]);exit;
...
detalle_asistencia_docente.CodigoProfesor = '" . $_cp . "' and detalle_asistencia_docente.idevento='" . $_id . "'";
    ...
    function insetar_externo($_P) {
        $sql = $this->Query("act_asistencia_externo(0,:p1,:p2,:p3,:p4)");
        $stmt = $this->db->prepare($sql);
        echo 'llegue';
        $stmt->bindValue(':p1', $_P['idevento'], PDO::PARAM_INT);
        $stmt->bindValue(':p2', $_P['id_externo'], PDO::PARAM_INT);
        $stmt->bindValue(':p3', $_P['cargo'], PDO::PARAM_INT);
        $stmt->bindValue(':p4', NULL, PDO::PARAM_STR);
...
and detalle_asistencia_externo.id_externos='$_ex'";
    ...
    function get_cursos($profesor, $evento,$semes) {
        $sql = "SELECT
...
WHERE carga_academica.CodigoSemestre='".$semes."' AND profesores.CodigoDptoAcad='07' AND profesores.CodigoProfesor='" . $profesor . "'";
//        echo $sql;Exit;
        $statement = $this->db->prepare($sql);
        $statement->execute();
        $data = $statement->fetchAll();
...
//        }
        ...
        try {
            ...
            foreach ($_P['CodigoAlumno'] as $key => $value) {
                ...
                $stmt2->bindValue(':p1', $CodigoAlumno, PDO::PARAM_STR);
                $stmt2->bindValue(':p2', $_P['idevento'], PDO::PARAM_INT);
                $stmt2->bindValue(':p3', 0, PDO::PARAM_STR);
                $stmt2->bindValue(':p4', $_P['cargo'][$key], PDO::PARAM_STR);
                $stmt2->bindValue(':p5', 'hola', PDO::PARAM_STR);
...
//        echo ($sql);exit;
    ...
    function get_datos_alumnnos_PS($idevento) {
        $sql = "SELECT
...
"
    ...
    function insert_profesor($_P) {
        ...
        try {
            ...
            foreach ($cursoA as $key => $value) {
                ...
                $stmt2->bindValue(':p5', $_P['cargo'], PDO::PARAM_INT);
    ...
    function insert_profeso_sin_curso($_P) {
        echo "inser:s¿in profe";
        $sql = $this->Query("act_asistencia_docente(0,:p1,:p2,:p3,:p4,:p5)");
        $stmt = $this->db->prepare($sql);
        $stmt->bindValue(':p1', $_P['cod_prof'], PDO::PARAM_STR);
        $stmt->bindValue(':p2', $_P['idevento'], PDO::PARAM_INT);
        $stmt->bindValue(':p3', NULL, PDO::PARAM_NULL);
        $stmt->bindValue(':p4', NULL, PDO::PARAM_NULL);
        $stmt->bindValue(':p5', $_P['cargo'], PDO::PARAM_INT);
        $p1 = $stmt->execute();
        $p2 = $stmt->errorInfo();
        return array($p1, $p2[2]);
    }

    function extraer_curso_asi_docente($iddocente) {
        
    }

    function insertar_alumnos($_P, $alumnos) {
...
//        return array($p1 , $p2[2]);
        ...
        try {
            ...
            foreach ($alumnos as $key => $value) {
                ...
                $stmt2->bindValue(':p5', $_P[cod_prof], PDO::PARAM_STR);
