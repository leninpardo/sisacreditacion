...
class crear extends Main{    
 ...
 function indexBuscarAlumno($query,$p,$c,$fac) 
    { //echo $fCac;
        ...
        $semestre=$ct['semestre_actual'];
         ...
         $sql = "SELECT 
                ...
                alumnos.FechaIngreso,
                alumnos.CodAlumnoSira,
		case when detalle_asignacion_tutoria.CodigoProfesor is null then 'Falta' else 'Asignado' end Estado
                FROM
                alumnos Inner Join matricula ON alumnos.CodigoAlumno = matricula.CodigoAlumno 
                left  join detalle_asignacion_tutoria on detalle_asignacion_tutoria.CodigoAlumno = alumnos.CodigoAlumno

        where  ".$c." like :query"." and  alumnos.CodigoFacultad='".$fac."' and   matricula.CodigoSemestre=".$semestre." ";         
        $param = array(array('key'=>':query' , 'value'=>"%$query%" , 'type'=>'STR' ));
        $data['total'] = $this->getTotal( $sql, $param );
        $data['rows'] =  $this->getRow($sql, $param , $p );        
        $data['rowspag'] = $this->getRowPag($data['total'], $p );        
    ...
    function insert($_P ) {
        ...
        $sql = $this->Query("sp_alum_iu(0,:p1,:p2,:p3,:p4,:p5,:p6,:p7,:p8,:p9,:p10,:p11,:p12,:p13,:p14,:p15,:p16,:p17,:p18,:p19,:p20,:p21,:p22,:p23,:p24,:p25,:p26,:p27,:p28,:p29,:p30,:p31,:p32,:p33,:p34,:p35)");
        ...
        $stmt->bindValue(':p13', $_P['distrito'] , PDO::PARAM_INT);
    ...
    function update($_P ) {
        ...
        $sql = $this->Query("sp_alum_iu(1,:p1,:p2,:p3,:p4,:p5,:p6,:p7,:p8,:p9,:p10,:p11,:p12,:p13,:p14,:p15,:p16,:p17,:p18,:p19,:p20,:p21,:p22,:p23,:p24,:p25,:p26,:p27,:p28,:p29,:p30,:p31,:p32,:p33,:p34,:p35)");
        $stmt = $this->db->prepare($sql);
        if($_P['idpadre']==""){$_P['idpadre']=null;}
        
        $stmt->bindValue(':p1', $_P['CodigoAlumno'] , PDO::PARAM_STR);
        $stmt->bindValue(':p2', $_P['ApellidoPaterno'] , PDO::PARAM_STR);
        $stmt->bindValue(':p3', $_P['ApellidoMaterno'] , PDO::PARAM_INT);
        $stmt->bindValue(':p4', $_P['NombreAlumno'] , PDO::PARAM_STR);
        $stmt->bindValue(':p5', $_P['Direccion'] , PDO::PARAM_INT);
