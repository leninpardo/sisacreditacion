...
require_once '../model/eventoEU.php';
require_once '../model/ubigeos.php';

class eventoEUController extends Controller {
    ...
    public function index() {
        ...
        $data['query'] = $_GET['q'];
        $data['pag'] = $this->Pagination(array('rows' => $data['data']['rowspag'], 'url' => 'index.php?controller=eventoEU&action=index', 'query' => $_GET['q']));

        $cols = array("CODIGO", "TEMA DE EVENTO", "TIPO", "FECHA", "LUGAR");

        $opt = array("eventotema" => "Tema", "evento.fecha" => "Fecha");
        $data['grilla'] = $this->grilla_E_PS_EU("eventoEU", $cols, $data['data']['rows'], $opt, $data['pag'], true, true, false, true, true, false);
        $view = new View();
        $view->setData($data);
        $view->setTemplate('../view/eventoEU/_Index.php');
...
//        $data['pre_actividades'] = $this->leer_pre_actividades($idevento);
   ...
   public function show_detalles(){
        ...
        $data['tipo_evento'] = $this->Select(array('id' => 'idtipo_evento', 'name' => 'idtipo_evento', 'table' => 'tipo_evento where tipo_evento.idtipo_evento="3" or tipo_evento.idtipo_evento="5"', 'code' => $obj['idtipo_evento']));
        $view->setData($data);
        $view->setTemplate('../view/eventoEU/_descrDetalles.php');
        $view->setLayout('../template/Layout.php');
        $view->render();
    }
    ...
    public function unirse_evento() {
        ...
        $view->setData($data);
        $view->setTemplate('../view/eventoEU/_unirse_evento.php');
        $view->setLayout('../template/Layout.php');
        $view->render();
    }
    ...
    public function unirse_profesor_evento() {
        ...
        $data['semestre'] = $obj->mostrar_ultimo_semestre();
        $view->setData($data);
        $view->setTemplate('../view/eventoEU/_unirse_profesor_evento.php');
        $view->setLayout('../template/Layout.php');
        $view->render();
    }
    ...
    public function insertarDetalle() {
       ...
       print_r( $p[1]);
        header('Location: index.php?controller=eventoEU&action=index_alumno');
        $data = array();
        $view = new View();
        $view->setData($data);
    ...
    public function insertarDetalle_profesor() {
        ...
        header('Location: index.php?controller=eventoEU&action=index_profesor');
        $data = array();
        $view = new View();
        $view->setData($data);
        $view->setTemplate('../view/_Error_App.php');
...
//,"UNIRSE"
        $opt = array("eventotema" => "Tema", "evento.fecha" => "Fecha");
        $data['grilla'] = $this->grilla_E_PS_EU("eventoEU", $cols, $data['data']['rows'], $opt, $data['pag'], false, false, false, false, false, true);
        $view = new View();
        $view->setData($data);
        $view->setTemplate('../view/eventoEU/_Index.php');
        $view->setLayout('../template/Layout.php');
    ...
    public function index_alumno() {
        ...
        $cols = array("CODIGO", "TEMA DE EVENTO", "TIPO", "FECHA", "LUGAR");
//,"UNIRSE"
        $opt = array("eventoEU.DescripcionTipoColegio" => "Tema");
        $data['grilla'] = $this->grilla_E_PS_EU("eventoEU", $cols, $data['data']['rows'], $opt, $data['pag'], false, false, false, false, true, true);
        $view = new View();
        $view->setData($data);
        $view->setTemplate('../view/eventoEU/_Index.php');
        $view->setLayout('../template/Layout.php');
...
//,"UNIRSE"
        $opt = array("eventoEU.DescripcionTipoColegio" => "Tema");
        $data['grilla'] = $this->grilla_E_PS_EU("eventoEU", $cols, $data['data']['rows'], $opt, $data['pag'], true, true, false, true, true, false, true);
        $view = new View();
        $view->setData($data);
        $view->setTemplate('../view/eventoEU/_Index.php');
        $view->setLayout('../template/Layout.php');
    ...
    public function save() {
        ...
        if ($_POST['idevento'] == '') {
            ...
            if ($p[0]) {
                ...
                $view = new View();
                $data['msg'] = $p[1];
                $data['url'] = 'index.php?controller=eventoEU';
                $view->setData($data);
                $view->setTemplate('../view/_Error_App.php');
                echo $view->renderPartial();
            }
            ...
            if ($p[0]) {
                header('Location: index.php?controller=eventoEU');
            } else {
                $data = array();
                $view = new View();
                $data['msg'] = $p[1];
