...
require_once '../model/notificaciones_docentes_ps.php';
//require_once '../model/ubigeos.php';

class notificaciones_docentes_psController extends Controller {
...
////,"UNIRSE" 
 $data['notificaciones_docentes_ps']=  $this->grilla_notificaciones_docentes_ps(); 
...
//        $data['grilla'] = $this->grilla_E_PS_EU("evento_proyeccion_social", $cols, $data['data']['rows'], $opt, $data['pag'], false, true, false, false,false,false);
        $view = new View();
        $view->setData($data);
        $view->setTemplate('../view/notificaciones_docentes_ps/_Index.php');
 ...
 public  function Parametros ()
    ...
    public function insertarDetalle_docentes_notificaciones() {
        $obj = new notificaciones_docentes_ps();
        $obj->InsertDet_docentes_notificaciones($_REQUEST);
        header('Location: index.php?controller=notificaciones_docentes_ps&action=index');
        $data = array();
        $view = new View();
        $view->setData($data);
