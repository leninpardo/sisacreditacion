...
<style>
    ...
    <div id="dialogo2" style="" title="ACEPTAR">
        <form action="index.php" id="frm" method="POST">
            <input type="hidden" name="controller" value="solicitudes_docentes_ps" />
            <input type="hidden" name="action" value="save" />
            <h2>DESEA ACEPTAR AL DOCENTE?</h2>
            <input type="hidden" name="estado" value="aceptado">
            <textarea name="idevento" class="hidden"  id="id2"></textarea>
            <textarea name="CodigoProfesor" class="hidden" id="idalumno"></textarea>
            <textarea name="mensaje_confirmacion" ></textarea>
           <br> <span style="font-size: 10px; color: red">*Mensaje no es obligatorio</span>
    ...
    <div id="dialogo3" style="" title="RECHAZAR">
        <form action="index.php" id="frm" method="POST">
            <input type="hidden" name="controller" value="solicitudes_docentes_ps" />
            <input type="hidden" name="action" value="save" />
            <h2>DESEA RECHAZAR AL DOCENTE?</h2>
            <input type="hidden" name="estado" value="rechazado">
            <textarea name="idevento" class="hidden"  id="id3"></textarea>
            <textarea name="CodigoProfesor" class="hidden" id="idalumno3"></textarea>
            <textarea name="mensaje_confirmacion" ></textarea>
             <br> <span style="font-size: 10px; color: red">*Mensaje no es obligatorio</span>
        ...
        <tbody>
            <?php foreach ($rows as $key => $value) { ?>
                <tr>    
                    ...
                    <td>
                        <?php echo strtoupper(utf8_encode($value[7])); ?>
                    </td>
                            ...
                            <div id="abrir2" style="margin-left: 20px;">
                                <li style="margin: 2px;position: relative;padding: 4px 0;cursor: pointer;float: left;list-style: none; font-family: Calibri;" class="ui-state-default ui-corner-all" title=".ui-icon-circle-plus">
                                    <span style="float: left; margin: 0 4px; background-image: url(css/images/ui-icons_2e83ff_256x240.png);"class="ui-icon ui-icon-circle-check"
                                          onclick="Unirse('<?= strtoupper(utf8_encode($value[9])) ?>','<?= strtoupper(utf8_encode($value[5])) ?>')">
                                    </span>
                                </li>
                            </div> 
                        </td>
                            ...
                            <div id="abrir3" style="margin-left: 20px;">
                                <li style="margin: 2px;position: relative;padding: 4px 0;cursor: pointer;float: left;list-style: none; font-family: Calibri;" class="ui-state-default ui-corner-all" title=".ui-icon-circle-plus">
                                    <span style="float: left; margin: 0 4px; background-image: url(css/images/ui-icons_2e83ff_256x240.png);"class="glyphicon glyphicon-remove"
                                          onclick="Unirse2('<?= strtoupper(utf8_encode($value[9])) ?>','<?= strtoupper(utf8_encode($value[5])) ?>')">
                                    </span>
                                </li>
                            </div> 
                        </td>
